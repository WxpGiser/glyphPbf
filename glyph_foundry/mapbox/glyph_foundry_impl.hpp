...
namespace sdf_glyph_foundry
{
                                    ...
                                    const Point &w)
    {
      ...
      const float t = ((p.get<0>() - v.get<0>()) * (w.get<0>() - v.get<0>()) + (p.get<1>() - v.get<1>()) * (w.get<1>() - v.get<1>())) / l2;
      if (t < 0) return v;
      if (t > 1) return w;
                         ...
                         FT_Face ft_face)
    {
		...
		FT_Outline_Funcs func_interface;
		func_interface.move_to = &MoveTo;
		func_interface.line_to = &LineTo;
		func_interface.conic_to = &ConicTo;
		func_interface.cubic_to = &CubicTo;
        ...
        for (const Points &ring : user.rings) {
            ...
            for (; p2 != ring.end(); p1++, p2++) {
                const float segment_x1 = (std::min)(p1->get<0>(), p2->get<0>());
                const float segment_x2 = (std::max)(p1->get<0>(), p2->get<0>());
                const float segment_y1 = (std::min)(p1->get<1>(), p2->get<1>());
                const float segment_y2 = (std::max)(p1->get<1>(), p2->get<1>());
